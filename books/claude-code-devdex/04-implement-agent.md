...
DevDexでは637+件のPRがマージされた。すべてが1 Issue = 1 PRの原則を守っている。この一貫性がReview Agentのレビュー品質を支えた。レビュー対象が小さければ、見落としも少なくなる。
...
Implement Agentがコードを書いたら、次はTest Agentの出番だ。第5章では「コードを編集できないテストエンジニア」が、2,350件のテストをどう書いたかを解説する。
