...
## DBスキーマ: 35マイグレーションの全体像

35本のマイグレーションで構築されたDevDexのDBは、6つの機能領域に分類できる。
...
Feature Gateは単なるアクセス制御ではなく、集客から課金までの導線そのものだ。

```
エンジニアタイプ診断（登録不要・64問）
  ↓ 結果ページでSNSシェア → バイラルループ
  ↓ 「詳細分析を見るには登録」→ 会員登録
  ↓ 用語を登録して使い込む
  ↓ 20件上限 or エクスポートしたい → Pro課金（¥500/月）
```

この導線のポイントは **「制限を壁ではなく試食にする」** 設計思想だ。

Freeユーザーが20件の用語を登録すると上限に到達する。しかし、20件分のデータは既にDevDexに蓄積されている。ここで「もう使えません」と壁を立てるのではなく、「20件分のデータを活かすには、もっと登録してスキルシートに反映しませんか？」とProの価値を見せる。**データを入れるのは無料、外に出す（PDF/Excel/公開URL）のは有料**。使い込むほどProの価値が上がる設計にした。

技術的には `FeatureGate` コンポーネントで一元管理している。Freeユーザーには機能のプレビュー（ぼかし表示や一部だけ表示）+「Proで使えます」のアップグレード導線を出す。完全にブロックするのではなく、「この先にある価値」を見せることで、試食として機能させる。
...
DevDexの64本のAPIは、すべて同一のレスポンス形式に従う。
...
**3. Implement Agentの一貫性**。パターンが1つに固定されているため、新しいAPIを追加するときもテンプレートのコピーで済む。64本のAPIすべてでレスポンス形式が統一されたのは、このパターンの効果だ。
...
35本のマイグレーションは、開発の歴史そのものだ。各マイグレーションがどのフェーズのどのissueで追加されたかを追えば、DevDexがどう設計されたかの全貌が分かる。
